...
@Stateless
@LocalBean
public class CdsCodeSystemDAO extends BaseDAO<CdsCodeSystemDTO> {
    ...
    protected void initialize() throws MtsException {
        ...
        this.registerDML(ByGeneralProperties.class, new QueryCallback(getDtoTableName()) {
            ...
            public String getQueryDML(BaseDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO) {
                setWildcardPredicateValue(baseDTO, " lower(code_system_id) like :text ", "text");
                setWildcardPredicateValue(baseDTO, " lower(oid) like :text ", "text");
                setWildcardPredicateValue(baseDTO, " lower(name) like :text ", "text");
                return getSelectDML() + getAndClearPredicateMap(" where ", "", Operator.OR);
            }
        ...
        this.registerDML(CdsCodeSystemDTO.ByOid.class, new QueryCallback(getDtoTableName()) {

            @Override
            protected String getQueryDML(BaseDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO) {
                return "select * from cds_code_system where lower(oid) = :oid";
            }
        }, false);

        this.registerDML(CdsCodeSystemDTO.ByLowerName.class, new QueryCallback(getDtoTableName()) {

            @Override
            protected String getQueryDML(BaseDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO) {
                return "select * from cds_code_system where lower(name) = :name";
            }
        }, false);
    }
}
